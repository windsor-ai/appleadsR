...
The goal of `appleadsR` is to help `R` users to access Apple Search Ads digital marketing data via `Windsor.ai` `API` in a convenient way from `R`.
...
You can install the development version of `appleadsR` from [GitHub](https://github.com/) with:

``` r
# install.packages("devtools")
devtools::install_github("pablosanchezmart/appleadsR")
```
...
The package currently has only one function `fetch_appleadsR` which will return a `data.frame` provided that all of the arguments are supplied to it: 
...
Will return a `data.frame` with Apple Search Ads marketing data.  
